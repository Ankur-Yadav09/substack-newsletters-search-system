...
>
> Updated 2026-07-28. Cross-reference [CLAUDE.md](CLAUDE.md) and [ARCHITECTURE.md](ARCHITECTURE.md)
...
- ✅ 31 passing tests: fully-mocked API layer (no live-service dependency), evaluation pipeline (9 tests), auth/rate-limit dependencies (8 tests), Gradio auth parsing (8 tests), integration tests with auto-provisioned test DB
...
1. **CI pipeline (`ci.yml`).** Cheap to add, and a portfolio project with 31+ passing tests and zero CI to run them looks unfinished. `ruff check` + `mypy` + `pytest` on every push/PR — set this up early so it protects everything built afterward.
2. **Re-ranking after retrieval.** The clearest "I understand RAG beyond the tutorial basics" signal. Ranking currently stops at RRF fusion; a cross-encoder re-rank step on the top candidates is well-known, well-regarded, and a natural interview talking point.
3. **Semantic caching.** Repeated/similar questions currently re-run the full embed → search → LLM pipeline every time. Start simple — an exact-query-string cache (even an in-memory TTL dict) as v1 — then upgrade to true semantic caching (check Qdrant for a near-duplicate *past* query above a similarity threshold before calling the LLM at all) as v2. Good cost/latency story, and a legitimate scaling technique worth having on a resume.
4. **Retry/backoff on LLM provider calls.** Small, concrete reliability improvement, easy to explain and demonstrate ("what happens when OpenRouter blips").
5. **A small golden eval dataset (~20 query/answer pairs).** Completes the evaluation story you already built. Right now you have live scoring; adding regression testing ("did this prompt change make things worse?") turns it into a genuinely complete MLOps narrative.
6. **Date-range filtering.** Small, natural feature completion; `published_at` is already stored, just not exposed as a filter.
7. **Ship it to AWS (App Runner) — not Cloud Run as README.md describes.** By far the biggest deployment differentiator. "Built a RAG system" is good; "built and deployed a RAG system, here's the live URL" is what actually stands out. Needs: fix `reload=True` in `src/api/main.py` first (dev-only flag, must not run in a deployed container), then ECR + App Runner for the backend, containerize Gradio as a second small service. This is the README's most out-of-date claim — worth fixing there too once deployed. Deliberately placed here, after the app is feature-complete and hardened — deploying earlier just means redeploying repeatedly as the app above changes.
8. **CD pipeline (`cd.yml`).** Builds on #7 and #1: once there's a real deployment and CI is green, automate it — build the Docker image, push to ECR, redeploy the App Runner service, triggered on merge to `main` (gated on CI passing first). This is what turns "I deployed it once by hand" into "I have a real deployment pipeline," a meaningfully stronger resume claim.
9. **Fully automated data ingestion pipeline.** Right now RSS + embedding ingestion only run via manual `make` commands — not "production" by any real definition. The project already has Prefect wired in with `deploy-cloud-flows`/`deploy-local-flows` Makefile targets, but the `prefect-cloud.yaml` file they reference doesn't exist yet. Concretely: write `prefect-cloud.yaml` defining both flows with cron schedules, deploy via the existing `make deploy-cloud-flows` target, and run a worker (Prefect Cloud's managed execution if available on the free tier, otherwise one small persistent worker process) so new articles actually appear on their own, with no one running a command by hand. Saved for last since it's an operational concern independent of the app's own feature set.

## Nice-to-have (only after the above, don't block on these)

- Test coverage for `qdrant_vectorstore.py` and the individual provider services
...
- 🚫 **Query rewriting/HyDE, MMR diversity, citation-grounding validation** — legitimate RAG improvements, but diminishing returns past what's needed to demonstrate competence given re-ranking and semantic caching are already prioritized above.
- 🚫 **Circuit breakers / cross-provider automatic fallback** — retry/backoff (already prioritized) captures most of the reliability value; a full fallback chain is more complexity than a personal project's traffic justifies.
...
1. ~~Gradio auth~~ ✅ done
2. CI pipeline — protects every change made from here on
3. Re-ranking
4. Semantic caching
5. Retry/backoff
